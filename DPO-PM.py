...
def preference_loss(policy_chosen_logps: torch.FloatTensor,
                    ...
                    power_mean_p: Optional[float] = None,
                    power_mean_lambda: float = 1.0) -> Tuple[torch.FloatTensor, torch.FloatTensor, torch.FloatTensor]:
    """Compute the preference loss for a batch of policy and reference model log probabilities.
    Supports DPO, IPO, and Power Mean loss variants.
    ...
    Args:
        ...
        beta: Temperature parameter for the DPO loss, typically in the range of 0.1 to 0.5.
        label_smoothing: Conservativeness for DPO loss, assuming preferences are noisy.
        ipo: If True, use the IPO loss instead of the DPO loss.
        reference_free: If True, ignore the provided reference model and use uniform reference.
        power_mean_p: If set (>1), use Power Mean loss with this exponent.
        power_mean_lambda: Coefficient for aligning reward differences in Power Mean loss.
    ...
    # Calculate reference log ratios if needed
    pi_logratios = policy_chosen_logps - policy_rejected_logps
    ref_logratios = reference_chosen_logps - reference_rejected_logps if not reference_free else 0
    logits = pi_logratios - ref_logratios

    # Compute rewards (consistent across loss functions)
    chosen_rewards = beta * (policy_chosen_logps - reference_chosen_logps).detach()
    rejected_rewards = beta * (policy_rejected_logps - reference_rejected_logps).detach()

    # Select the appropriate loss function
    if power_mean_p is not None and power_mean_p > 1:
        # Power Mean loss implementation based on the given formula
        # Calculate the power mean term
        power_mean = torch.pow(
            (torch.pow(policy_chosen_probs, power_mean_p) + torch.pow(policy_rejected_probs, power_mean_p)) / 2,
            1 / power_mean_p
        )
        
        # Calculate the reward difference term
        reward_diff_term = power_mean_lambda * (policy_chosen_probs - policy_rejected_probs) / 2
        
        # The loss is the negative of the power mean minus the reward difference term
        # We use negative because we want to maximize the power mean while aligning with rewards
        losses = - (power_mean - reward_diff_term)
...
def concatenated_inputs(batch: Dict[str, Union[List, torch.LongTensor]]) -> Dict[str, torch.LongTensor]:
    ...
    Args:
        batch: A batch of data. Must contain 'chosen_input_ids' and 'rejected_input_ids'.
        
    Returns:
        A dictionary containing the concatenated inputs under 'concatenated_input_ids'.
    """
...
class BasicTrainer(object):
    def __init__(self, policy: nn.Module, config: DictConfig, seed: int, run_dir: str, reference_model: Optional[nn.Module] = None, rank: int = 0, world_size: int = 1):
        """A trainer for a language model, supporting SFT, DPO, and Power Mean loss training."""
        self.seed = seed
        self.rank = rank
    ...
    def get_batch_samples(self, batch: Dict[str, torch.LongTensor]) -> Tuple[str, str]:
        ...
        if self.config.loss.name in {'DPO-PM', 'ipo', 'DPO-PM'}:
        ...
        if self.config.loss.name in {'DPO-PM', 'ipo', 'power_mean'}:
    ...
    def concatenated_forward(self, model: nn.Module, batch: Dict[str, Union[List, torch.LongTensor]]) -> Tuple[torch.FloatTensor, torch.FloatTensor]:
            # batchNone
        if batch is None:
            print("❌❌❌ ERROR: batch is None in concatenated_forward")
        ...
        for key in required_keys:
            if key not in batch:
                print(f"❌❌❌ ERROR: Missing key {key} in batch")
                device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
                return torch.zeros(1, device=device), torch.zeros(1, device=device)
        
        # batch
        if batch['chosen_input_ids'].shape[0] == 0 or batch['rejected_input_ids'].shape[0] == 0:
            print(f"❌❌❌ ERROR: Empty batch! chosen: {batch['chosen_input_ids'].shape}, rejected: {batch['rejected_input_ids'].shape}")
            device = batch['chosen_input_ids'].device
            bs = max(batch['chosen_input_ids'].shape[0], batch['rejected_input_ids'].shape[0], 1)
            return torch.zeros(bs, device=device), torch.zeros(bs, device=device)
    ...
    def get_batch_metrics(self, batch: Dict[str, Union[List, torch.LongTensor]], loss_config: DictConfig, train=True):
        ...
        if loss_config.name in {'DPO-PM', 'ipo', 'power_mean'}:
            policy_chosen_logps, policy_rejected_logps = self.concatenated_forward(self.policy, batch)
            with torch.no_grad():
                reference_chosen_logps, reference_rejected_logps = self.concatenated_forward(self.reference_model, batch)

            # Common loss arguments
            base_loss_kwargs = {
            ...
            if loss_config.name == 'DPO-PM':
                loss_kwargs = {**base_loss_kwargs, 'ipo': False}
            elif loss_config.name == 'ipo':
                loss_kwargs = {** base_loss_kwargs, 'ipo': True}
            elif loss_config.name == 'power_mean':
                loss_kwargs = {
                    **base_loss_kwargs,
                    'power_mean_p': loss_config.get('p', 2.0),  # Default to p=2 for small models
                    'power_mean_lambda': loss_config.get('lambda', 1.0)  # Alignment coefficient
                }
            else:
...
class FSDPTrainer(BasicTrainer):
    def __init__(self, policy: nn.Module, config: DictConfig, seed: int, run_dir: str, reference_model: Optional[nn.Module] = None, rank: int = 0, world_size: int = 1):
        ...
        if config.loss.name in {'DPO-PM', 'ipo', 'power_mean'}:
