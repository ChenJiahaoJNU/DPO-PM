...
def extract_anthropic_prompt(prompt_and_response: str) -> Union[str, None]:
    """Assistant:None"""
    search_term = "\n\nAssistant:"
    search_term_idx = prompt_and_response.rfind(search_term)
    # None
...
def get_hh(split: str, silent: bool = False, cache_dir: str = None) -> Dict[str, Dict[str, Union[List[Tuple[int, int]], List[str], str]]]:
    """Load the Anthropic Helpful-Harmless dataset from Parquet"""
    print(f'Loading HH dataset ({split} split) from Parquet...')

    # 1. 
    dataset_root = Path("/root/autodl-fs/2025autumn/HF_Datasets/HH6")
    data_dir = dataset_root / "data"

    # 2. Parquettrain/test
    if split == "train":
        file_pattern = str(data_dir / "train-*.parquet")
    elif split == "test":
        file_pattern = str(data_dir / "test-*.parquet")
    else:
        raise ValueError(f"split'train''test'{split}")

    # 3. 
    full_dataset = load_dataset(
        "parquet",
        data_files={split: file_pattern},  # 
        cache_dir=cache_dir  # 
    )
    ...
    def split_prompt_and_responses(ex):
        # extract_anthropic_promptNone
        prompt = extract_anthropic_prompt(ex['chosen'])
        if prompt is None:  # Assistant:
            return None, None, None
        # 
        chosen_response = ex['chosen'][len(prompt):].strip()
        rejected_response = ex['rejected'][len(prompt):].strip()
        # 
    ...
    valid_samples = total_samples - invalid_count
    print(f"HH{total_samples} → {valid_samples} → {invalid_count}")
