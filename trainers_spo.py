...
def preference_loss(policy_chosen_logps: torch.FloatTensor,
    ...
    """
    SPO
    SPOL_SPO = - (β·logits)·exp(-β·logits)logits=1/β
    """
    ...
    logits = pi_logratios - ref_logratios  # SPOh_πθ(y_w,y_l)
    
    # SPOSPO
    z = beta * logits  # z=1logits=1/β
    losses = -z * torch.exp(-z)
...
class BasicTrainer(object):
    def __init__(self, policy: nn.Module, config: DictConfig, seed: int, run_dir: str, reference_model: Optional[nn.Module] = None, rank: int = 0, world_size: int = 1):
        ...
        data_iterator_kwargs = dict(
            ...
            sft_mode=False,  # SPOSFT
        )

        self.policy = policy
        self.reference_model = reference_model
        if self.reference_model is None:
            raise ValueError("SPO(reference_model)")
    ...
    def get_batch_metrics(self, batch: Dict[str, Union[List, torch.LongTensor]], loss_config: DictConfig, train=True):
        ...
        # SPOlogits
        pi_logratios = policy_chosen_logps - policy_rejected_logps
        ref_logratios = reference_chosen_logps - reference_rejected_logps if not loss_config.reference_free else 0
        logits = pi_logratios - ref_logratios
        target_logits = 1.0 / loss_config.beta  # SPO
        metrics[f'logits_{train_test}/current'] = logits.detach().cpu().numpy().tolist()
        metrics[f'logits_{train_test}/target'] = [target_logits] * len(logits)
        metrics[f'logits_{train_test}/diff'] = (logits - target_logits).detach().cpu().numpy().tolist()
...
class FSDPTrainer(BasicTrainer):
    def __init__(self, policy: nn.Module, config: DictConfig, seed: int, run_dir: str, reference_model: Optional[nn.Module] = None, rank: int = 0, world_size: int = 1):
        ...
        assert config.model.block_name is not None, 'FSDPmodel.block_nameGPT2BlockGPTNeoXLayer'
